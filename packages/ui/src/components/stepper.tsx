  ...
  return {
    ...
    Stepper: {
      Provider: ({
        ...
        const { initialStep, initialMetadata, ...rest } = props;
        ...
        return (
          ...
          >
            <Scoped initialStep={initialStep} initialMetadata={initialMetadata}>
              <StepperContainer className={className} {...rest}>
                {children}
              </StepperContainer>
            </Scoped>
          </StepperContext.Provider>
          ...
          return (
            ...
            >
              ...
              <div
                ...
                className="flex flex-col items-start gap-3xsmall"
              >
                {title}
                {description}
              </div>
        ...
        return (
          <>
            <li
              date-component="stepper-step"
              className={cn([
                'group peer relative flex items-center gap-small',
            ...
            >
              <Button
                ...
                className={cn(
                  'rounded-full size-7 h4-primary m-[3px]',
                  dataState === 'inactive' && !isCompleted && 'bg-neutral-700',
                  className,
                )}
                variant={stepBtnVariant}
                size="icon"
                ...
                {...props}
              >
                {!isCompleted && (icon ?? stepIndex + 1)}
                {isCompleted &&
                  (dataState !== 'active'
                    ? (completedIcon ?? stepIndex + 1)
                    : (icon ?? stepIndex + 1))}
              </Button>
              ...
              <div
                date-component="stepper-step-content"
                className={cn(
                  'flex flex-col items-start gap-3xsmall',
                  dataState === 'inactive' && 'text-neutral-900',
                  isActive && '[&>p]:!text-neutral-1000',
                )}
              >
                {title}
                {description}
              ...
              <div className="flex gap-small">
                {!isLast && (
                  <>
                    ...
                    <div className={cn('my-3 flex-1 ps-4', separatorClass)}>
                      {panel}
                    </div>
                  </>
                )}
...
const Title = ({
  ...
  return (
    <Comp
      ...
      className={cn('h4-primary text-neutral-1000', className)}
      {...props}
    >
      {children}
    </Comp>
...
const classForNavigationList = cva('flex gap-medium', {
  variants: {
    variant: {
...
const classForSeparator = cva(
  [
    'bg-neutral-500',
    'data-[state=completed]:bg-neutral-500',
    'transition-all duration-300 ease-in-out',
  ],
  {
    variants: {
      orientation: {
        horizontal: 'h-[1px] flex-1',
    ...
    Stepper: {
      ...
      Step: (
        props: React.ComponentProps<'button'> & {
          ...
          stepBtnVariant?: VariantProps<typeof buttonVariants>['variant'];
          containerClass?: string;
          separatorClass?: string;
          isCompleted?: boolean;
        },
      ) => React.ReactElement;
      Title: (props: AsChildProps<'div'>) => React.ReactElement;
